...
async def _async_process_media(user_id, s3_key, platform_payload, **kwargs):
    ...
    try:
        ...
        await send_telegram_message(chat_id, "❌ Ошибка во время транскрипции.")
        return
    ...
    async def process_option(option_code):
        ...
        try:
            ...
            await send_telegram_message(chat_id, f"❌ Ошибка при обработке: *{title}*")

    processing_tasks = [process_option(option) for option in selected_options]
    await asyncio.gather(*processing_tasks)
    await send_telegram_message(chat_id, "🎉 Обработка завершена!")
...
def process_media_v2(user_id, s3_key, metadata, platform_payload, **kwargs):
    try:
        asyncio.run(_async_process_media(user_id, s3_key, platform_payload, **kwargs))
    except Exception as e:

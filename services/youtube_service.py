    ...
    def download_audio(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with yt_dlp.YoutubeDL(self.ydl_opts) as ydl:
                ...
                downloaded_path = ydl.prepare_filename(info).replace(info['ext'], 'mp3')
            ...
            # Проверяем специфичную ошибку размера
            if "File size exceeds" in str(de):
                return {"error": "File size exceeds the 20MB limit."}
            return {"error": "Failed to download video. It might be private or unavailable."}
